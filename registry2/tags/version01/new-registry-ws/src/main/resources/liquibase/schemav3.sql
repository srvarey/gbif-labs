...
--
CREATE FUNCTION assert_min_length(input text, minlength integer) RETURNS boolean AS $$ BEGIN IF (char_length(trim(input))) >= minlength THEN RETURN TRUE; ELSE RETURN FALSE; END IF; END; $$ LANGUAGE plpgsql; 
...
(
  ...
  language varchar(50) NOT NULL CHECK (assert_min_length(language, 2)),
  ...
  country varchar(50) CHECK (assert_min_length(country, 2)),
...
(
  ...
  description text CHECK (assert_min_length(description, 10)),
  language character(50) NOT NULL CHECK (assert_min_length(language, 2)),
  email varchar(254) CHECK (assert_min_length(email, 5)),
  phone varchar(50) CHECK (assert_min_length(phone, 5)),
  homepage text CHECK (assert_is_http(homepage)),
  logo_url text CHECK (assert_is_http(logo_url)),
