 ...
 */
.controller('OrganizationCtrl', function ($scope, $state, $stateParams, notifications, Restangular, DEFAULT_PAGE_SIZE) {
  ...
  var load = function() {
    ...
    .then(function(organization) {
      ...
      $scope.counts.comment = _.size(organization.comments); 
      
      organization.getList('ownedDataset', {limit: DEFAULT_PAGE_SIZE})
        .then(function(response) {
          $scope.ownedDatasets = response.results;
          $scope.counts.ownedDatasets = response.count;
        });
      ...
      organization.getList('hostedDataset', {limit: DEFAULT_PAGE_SIZE})
        .then(function(response) {
          ...
          $.each(response.results, function(index, dataset) {
            Restangular.one('organization', dataset.owningOrganizationKey).get().then(function(organization) {
  ...
  $scope.cancelEdit = function () {
    $scope.organization = Organization.get({ key: key });
    $scope.transitionTo("detail");
  }
