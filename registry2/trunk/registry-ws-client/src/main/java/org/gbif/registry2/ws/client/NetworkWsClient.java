...
public class NetworkWsClient extends BaseWsGetClient<Network, UUID> implements NetworkService {
  ...
  public NetworkWsClient(@RegistryWs WebResource resource) {
    super(Network.class, resource.path("network"), null);
  }
  ...
  public PagingResponse<Network> list(Pageable page) {
    return get(GenericTypes.PAGING_NETWORK, null, null, page);
  }
  ...
  public PagingResponse<Network> search(String query, Pageable page) {
    return get(GenericTypes.PAGING_NETWORK, (Locale) null, QueryParamBuilder.create("q", query).build(), page);
  }
}
