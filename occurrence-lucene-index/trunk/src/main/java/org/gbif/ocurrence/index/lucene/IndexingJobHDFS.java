...
public class IndexingJobHDFS extends Configured implements Tool {
  ...
  public static class Indexer extends Mapper<LongWritable, Text, IntWritable, Text> {
    ...
    public void map(LongWritable key, Text value, final Context context) throws InterruptedException, IOException {
      ...
      Path destinationPath = new Path(_conf.get("finalDestination"));
      Path destinationShardPath = new Path(_conf.get("finalDestination") + sharId);
      FileSystem fs = FileSystem.get(destinationPath.toUri(), _conf);

      FileSystemDirectory fileSystemDirectory = new FileSystemDirectory(fs, destinationShardPath, true, _conf);
      IndexWriter indexWriter = new IndexWriter(fileSystemDirectory, analyzer, MaxFieldLength.UNLIMITED);
      ...
      t.interrupt();
      context.write(new IntWritable(sharId), new Text(destinationShardPath.getName()));
    }
  ...
  public enum OccurrenceFields {
    id, data_provider_id, data_resource_id, institution_code_id, collection_code_id, catalogue_number_id, taxon_concept_id, taxon_name_id, kingdom_concept_id, phylum_concept_id, class_concept_id, order_concept_id, family_concept_id, genus_concept_id, species_concept_id, nub_concept_id, iso_country_code, latitude, longitude, cell_id, centi_cell_id, mod360_cell_id, year, month, occurrence_date, basis_of_record, taxonomic_issue, geospatial_issue, other_issue, deleted, altitude_metres, depth_centimetres, modified;
  }
